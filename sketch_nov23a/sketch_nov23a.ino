...
char *mqttServer = "21d87a63d7064472b2615fc6e38d6919.s1.eu.hivemq.cloud";
int mqttPort = 8883;
const char *mqtt_username = "tiendat";
const char *mqtt_password = "iot2024";
const char *WifiSSID = "ĐạtHuy";
const char *WifiPassword = "dathuy03";
const char *topicSubcribe = "iot_hust/command";
const char *topicPublish = "iot_hust/data";
const char *ledId1 = "640ee113bf49af0a529390ad"; 
const char *ledId2 = "640ee119bf49af0a529390b1";

//mấy cái id này thì t cx chưa rõ lắm là để làm gì
...
//cái này sau sẽ chính là device id để gửi lên mqtt
const char *roomId1 = "62ce96c7cd95012e5f7155e1";
const char *roomId2 = "62ce96cfcd95012e5f7155e4";
//sau ví dụ có 3 4 5 device(device ý t ở đây là 1 bộ gồm esp dht mq2 còi đèn) thì chỉ cần code nạp vào mỗi cái có roomId khác nhau là đc
...
void setup() {
  ...
  sendingValueSensor["roomId"] = roomId1;//code này gán id room 1 cho con esp này, con còn lại sẽ là roomid2 
...
void loop(){
  ...
  button1.loop();
  button2.loop();
  //đổi logic của 2 cái nút này thành bật tắt còi
  if (button1.isPressed())
  {
    state1 = !state1;
    digitalWrite(LED1, state1);
    sendingStatusLed["roomId"] = roomId1;//chỗ này send json đi cũng để id là 1 để viết lên database sau này web đọc mới phân biệt đc là của phòng nào, con esp còn lại mã sẽ là roomid2
...
//call back này là nhận message từ mqttt, mấy a này dùng để điều khiển đèn nma mình sẽ dùng để tắt còi, logic tương tự, mình cũng có thể vừa để còi kêu vừa để đèn nhấp nháy cũng được khi đấy thì điều khiển tương tác cả 2
